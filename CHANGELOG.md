...
- Removed automatic `NICK` retries from `pingLoop`; the keepalive loop now only sends `PING`.
- Clear pending post-registration nick changes back to the confirmed current nick when `AutoNickRecoveryPostRegistration` is disabled and the server returns 432/433/436/437.
- Align local desired nick state with the server-confirmed nick after successful self `NICK` events.
...
[1.3.1]: https://github.com/kofany/go-ircevo/compare/v1.3.0...v1.3.1
